...
def generate_profile(user_id, reviews):
    ...
    profile = {
        ...
        "user_summary": generate_summary(profile),
...
def analyze_batch_and_generate_profiles(reviews_batch, db):
    ...
    for review in reviews_batch:
        ...
        db["users"].update_one(
            ...
            {"$setOnInsert": {"user_id": review["user_id"]}},
            upsert=True
        )
        # Cadastrar produto
        db["products"].update_one(
            {"product_id": review["product_id"]},
            {"$setOnInsert": {"product_id": review["product_id"]}},
            upsert=True
        )
...
if __name__ == "__main__":
    example_reviews = [
        {
            ...
            "comment": "Absolutely amazing product! Loved the design and performance.",
            "rating": 5,
            "sentiment": "positive",
            "spam": False
        },
        {
            "user_id": "user_demo",
            "product_id": "prod_test",
            "comment": "Customer support was slow. Not happy.",
            "rating": 2,
            "sentiment": "negative",
            "spam": False
        }
    ...
    db = MongoClient("mongodb://localhost:27017")["reviews_platform"]
    analyze_batch_and_generate_profiles(example_reviews, db)
